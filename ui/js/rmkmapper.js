...
$(document).ready(function() {
	...
	var loading_data = $.when(		
		...
		fetch_json("rmk.json", function(data) { _g.rmk_objects = data; })
	);
...
function apply_behavior() {
	...
	$("#acc_filter input:checkbox").change(function() {
		...
		$.each(_g.rmk_objects, function(i, o) {
			if (o.type != type || !o.marker) return true;
			o.marker.setVisible(show);
		});
	});
...
function apply_rmk_objects(rmk_objects, map) {
	...
	// Apply objects to map. The objects array is modified -
	// Marker and InfoWindow instances are added to each object
	// with coordinates.

	$.each(rmk_objects, function(i, o) {
		var o = rmk_objects[i];
		var marker = o.marker;
		var infow = o.infowindow;		
		var regio_href;
		var campicons;
		
		if (!o.coord_lat || !o.coord_lon)
			return true;
		
		if (!marker) {
			marker = new StyledMarker({
				styleIcon : get_styledicon(o.type),
				position : new google.maps.LatLng(o.coord_lat, o.coord_lon),
				map : map
			});
			o.marker = marker;
		}
		...
		if (!infow) {		
			...
			regio_href = 
				...
				o.coord_x + "," + o.coord_y + "," + o.coord_x + "," + o.coord_y;				
			infow = new google.maps.InfoWindow({
				content : 
					"<div class=\"iwcontent\">" + 
					"<b>" + o.name + "</b><ul>" +
					(o.location  ? "<li>" + o.location  + "</li>" : "") +
					(o.equipment ? "<li>" + o.equipment + "</li>" : "") +
					(o.sights    ? "<li>" + o.sights    + "</li>" : "") +						
					"</ul><a href=\"" + o.href + "\">RMK</a>" + " | " +
